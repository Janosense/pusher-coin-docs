...
## 2026-09-21 — The transport delivers through its own endpoint, on two schedulers, and never advances its cursor past an event it could not deliver
- **Context:** `realtime` Sprint 1 Step 5 built the transport the 2026-09-18 spike chose. That entry fixed *what* to poll, the `event_key` formula and the arithmetic; it left open how the polled events reach the wallet, what runs the schedule on the production host, and what a pass does when something goes wrong. Those three are decided here. Nothing in the spike is reopened.
- **Decision — it posts to `POST /pc/v1/machine/events` rather than calling `Machine_Ingest_Service` directly,** dispatched in-process with `rest_do_request()` and presenting `PC_MACHINE_INGEST_SECRET` like any other caller. The sprint's own verification for this step says the ingest secret must exist in production "or the first real event is lost", and Step 4's controller already describes the transport as a caller its rate limit may throttle — so the endpoint was always meant to be the door the transport knocks on. Going through it means the validation, the audit row, the ceiling and the `event_key` idempotency apply to the transport too, and a future transport outside WordPress changes nothing. In-process rather than over loopback HTTP because a loopback POST on shared hosting is a failure mode with nothing to do with the machine; the route, its permission callback and its body handling are identical either way.
- **Decision — both schedulers, because which one the host has is not knowable from here.** The spike left "a real cron or traffic-driven WP-Cron?" to this step, and it cannot be answered without the host. So the feature bootstrap schedules a WP-Cron event *and* `wp pc machine-poll` exists for a real cron line; a transient lock stops them overlapping and `event_key` would catch it anyway. `pc_realtime_poll_last_run` records which one ran, so the question is answerable on a host where nobody has a shell. WP-Cron's traffic-dependence bites least where it matters most: a player holding a turn has the room polling every 3 s.
- **Decision — a failed delivery holds the cursor.** Re-reading a window costs nothing (`event_key` makes the repeat free) and skipping one is a payout a player never got, so the cursor advances only past rows the endpoint has answered. A 401, a 429, a 500, an unreachable machine, a missing secret or a missing machine id all leave it where it was and write a `machine_poll_*` row to the audit log; a lost cursor costs a bounded backfill and an audit row. Every gap is recorded, none is silent.
- **Decision — `pc_realtime_poll_machine_id` is required, with no default.** It must match the `pc_room_machine_id` the live rooms carry, and no value can be guessed. Delivering events with an empty machine id would file every real payout as belonging to nobody and retire it unpaid, so an unset one is treated exactly like an unset secret: hold the cursor, record why, credit nothing. Both are then recoverable for as long as Home Assistant's ten-day retention lasts.
- **Consequences:**
  - **Timestamps are compared with microseconds kept.** `strtotime()` truncates to the second and Home Assistant writes microseconds; the coin counter rises in bursts, so two changes inside one second are ordinary and truncation dropped the second one unpaid. Found by the step's own checks.
  - **The ingest ceiling is shared with the transport.** A flood that spends the window throttles the poller too — harmless, because the pass stops and the next one resumes from the cursor, which is what Step 4's entry already assumed.
  - **Only `sensor.coin` increments are credited,** per the spike. The bonus map stays configured, tested and unreachable until a bonus is observed.
  - **Two things no deploy can write** must exist on the host before a real event pays: the secret in `wp-config.php` and the machine id option. Until then the transport is running and holding, not failing silently.
  - **The schedule is verified only by a real host.** No check command can execute a cron line or prove WP-Cron ticks; `wp pc machine-poll --status` is what answers it in production.


---

## 2026-09-21 — The push channel: a token request signed here, a room payload with no money, and a publish that cannot fail the payout
- **Context:** `realtime` Sprint 2 Step 1 built the push leg the 2026-09-15 entry chose. That entry fixed the provider, the free-tier ceilings, the key-is-a-wp-config-constant rule and "the SPA needs a token endpoint rather than the key". It left open how the pass is minted, what a pass may cover, what a room message carries, and what a failed publish does. Those four are decided here. Nothing in the 2026-09-15 entry is reopened, and no SDK is added — `vendor/` still never reaches production (2026-09-17, the Stripe precedent).
- **Decision — WordPress signs an Ably *token request* rather than fetching a token.** `GET /pc/v1/realtime/token` returns `{keyName, ttl, capability, clientId, timestamp, nonce, mac}`, the `mac` an HMAC-SHA256 over Ably's canonical field order keyed with the secret half of `PC_ABLY_KEY`; the browser exchanges it with Ably itself. The alternative — WordPress calling Ably's `requestToken` and passing the result on — puts an outbound HTTP call on a request a player is waiting for, with its own latency and its own failure mode, for no gain. Signing is pure HMAC, the same hand-written shape the Stripe webhook already verifies with.
- **Decision — a signed-in caller is granted every room channel at once, and only an administrator the machine channel.** Rooms are public (`DOMAIN.md`: anyone may watch a broadcast and read a room's chat), so `subscribe` on `{prefix}:room:*` reveals nothing a player could not already see, and it saves re-issuing a pass each time they open a different room. `{prefix}:machine` is operator detail and is added only for `manage_options`. **Nothing is ever granted `publish`:** the browser is untrusted and everything on these channels originates on the server.
- **Decision — the room payload carries no money.** A `credit` message names `room_id`, `user_id`, `coins`, `event_id` and `at`. A room channel is readable by everyone watching that room, and the price another player bought their coins at is not theirs to see, so `unit_price` stays behind the authenticated endpoints the winner's own totals already come from. The check asserts the absence by field name, so widening the payload later is a deliberate act rather than a drift.
- **Decision — an unconfigured key is a silent no-op, not a logged failure.** With no `PC_ABLY_KEY` the publisher attempts nothing and records nothing, and the token endpoint answers `realtime_not_configured` 503. An install with no Ably account should credit players normally rather than write one audit row per payout forever. A *configured* key that then fails is the opposite: caught, recorded as `realtime_publish_failed`, and swallowed.
- **Consequences:**
  - **The publish sits after the wallet move and can never reach back to it.** `publish()` returns `false` and never raises: a caller on the money path must not be handed an error. The timeout is 2 s so a black-holed connection costs a poller pass two seconds rather than thirty.
  - **Channel names live in one file** (`app/realtime/channels.php`) and are handed to the SPAs by the token endpoint, so neither hardcodes them and `pc_realtime_channel_prefix` stays safe to change — which is what lets one Ably app host staging and production without them hearing each other.
  - **One channel per room, one for the machine, never one per viewer** — the free tier caps an app at 200 channels, which a per-viewer channel would pass at the 201st player.
  - **The token TTL is clamped** to 60 s … 24 h, Ably's own maximum, whatever the option says.
...
## 2026-09-21 — The relay is the machine's service switch, and an **open** relay is what refuses a toss
- **Context:** `realtime` Sprint 2 Step 3 ("the relay lock the player can see"). Its own fixed decision sends it to the spike entry of 2026-09-18, which recorded three things and left the fourth open: the contact sensor is `sensor.relay_on` (`sensor.sw_b_t_relay` read `0` for ten days, including while the relay was switched); `1` means closed and `0` open, by the documented button names; and **the relay idles closed and never moved through 28 rises of the coin counter** — it follows only the two relay buttons someone at the venue presses. That entry then said, in so many words, that the toss lock was wrong: `POST /rooms/{id}/play` read the normal `1` as "the machine is mid-payout" and answered 423 to **every toss while the machine was on**. It routed the fix to `/adhoc` and observed that Step 3 had "no payout signal to follow". Nobody ran that `/adhoc`, and the step could not deliver a button that meant anything until the rule underneath it was settled.
- **Decision — the lock is the relay being *open*, and it lives in this step, not in an `/adhoc`.** `play()` refuses with `relay_open` 423 while `sensor.relay_on` reads `0`: the machine has been taken out of service by hand. Closed is the machine working, so a closed relay refuses nothing. The button is a mirror of the server rule, and mirroring a broken rule would have shipped a permanently disabled button, so the rule is this step's subject rather than a hotfix beside it.
- **Decision — the relay is watched on the poll pass that already exists.** `Realtime_Relay_Watch` reads the sensor once per pass, caches a change in `pc_realtime_relay_state` and publishes `relay` `{room_id, locked, at}`. No second schedule, nothing new to deploy, and one extra state read a minute.
- **Decision — an unreadable relay is not a locked one.** A read that fails publishes nothing, changes nothing and audits `machine_relay_read_failed`. Guessing "locked" on a timeout would grey out a working machine's button on no evidence.
- **Decision — the push is a courtesy, the live read is the gate.** The queue envelope carries `machine_locked` from the cache so the button is right at first paint, and the channel corrects it afterwards; but `play()` reads Home Assistant itself on every toss. A message that is late, dropped or never sent costs a refused toss, never a lost coin.
- **Alternatives rejected:** *Drop the lock altogether* — defensible on the evidence (the relay carries no payout signal and nobody has watched a toss with it open), but it removes the protection against the failure mode that actually costs a player money: the coin is debited, Home Assistant answers 200 to the button press, and nothing happens physically. *Leave the server fix to `/adhoc` and build the button afterwards* — one extra round trip, two sessions editing the same three lines, and this step's own money checks unable to run in between; it buys only an earlier path to `main`, and the platform has no players yet. *A second, faster schedule for the relay* — a service state that changes a few times a month does not earn its own cron.
- **Consequences:**
  - **The game is playable again.** Until this entry every toss was refused with 423 whenever the machine was on. Ten of `tests/realtime-relay.php`'s first sixteen checks fail against that code.
  - **`relay_closed` 423 no longer exists**; it is `relay_open` 423. The only consumer was `PlaceBet.vue`.
  - **`DOMAIN.md`'s relay row is rewritten:** the relay is the service switch, not the payout gate, and nothing is ever credited from it — `pc_machine_relay_coin_count` and `ingest_relay_closed` stay unreachable, as the 2026-09-18 entry already required.
  - **The button lags a real transition by up to one poll interval** (`pc_realtime_poll_interval_seconds`, 60) plus Home Assistant's own 1.1 s. Inside that window the player's own toss is refused and the button greys out from the refusal.
  - **Nothing here presses a relay button.** `Machine_Service::relay_open()` / `relay_close()` remain unused by the product; the verification guide presses one once, on the user's explicit go-ahead, exactly as the step text sanctions.
  - **This does not settle what an open relay does to a toss physically.** Nobody has watched one. The lock is a precaution taken on the documented meaning of the buttons, and the verification guide says so.

---

## 2026-09-21 — Chat rides the room channel: the cursor becomes the catch-up, bodies may travel, and guests keep polling
- **Builds on:** 2026-09-07 "Chat: gated below play, cursor-read, moderated by hiding". That entry's decisions all stand — public cursor-based read, `require_chat_ready` write, moderation by hiding, account-wide mute. What changes is the cursor's *job*. The old entry is left as written.
- **Context:** `realtime` Sprint 2 Step 4. The queue moved onto the push channel in Step 2 and chat was the last 3-second poll in `frontend/src/`. Two things about chat are not true of the queue: its read is **public**, and **guests use it** — the room page is public and `RoomChat.vue` has rendered the conversation read-only to guests since Phase 3.
- **Decision — the `after` cursor stops being the transport and becomes the catch-up.** It is called once when a client opens a room and again on every reconnect, instead of every three seconds. The rule that it must be cursor-based rather than `LIMIT/OFFSET` matters *more* now, not less: the gap it has to cover is "while the client was disconnected", which is longer and less predictable than "between two polls".
- **Decision — a `chat` message carries the whole message; a `moderation` message carries an id and a state and never a body.** This is one rule, not two: **what may travel is what the read already gives away.** `GET /rooms/{id}/messages` is public, so a body on the channel reveals nothing to anybody; `GET /rooms/{id}/queue` is play-ready gated, which is why the `queue` message carries a version and no queue. A hidden message's text is precisely what must stop travelling, so moderation names an id.
- **Decision — guests keep the 3-second poll; the token endpoint is not widened.** A pass requires a signed-in caller (2026-09-21, the token entry), so a guest has none. Serving them on the channel would cost **one of Ably's 200 concurrent connections per guest**, on a free tier whose ceiling this sprint's own goal says to measure first. A guest's chat therefore stays exactly as live as it was, rather than freezing — which is the part that actually mattered.
- **Decision — one Ably connection serves several listeners.** `frontend/src/services/realtime.js` keeps one client and one channel per room and fans every message out to consumers registered by name. The free tier counts connections, not subscriptions, and a room has both stores listening at once.
- **Alternatives rejected:** *Publishing only a change ping for chat, as the queue does* — it would cost a round trip per message for content the API already serves to anyone, and the permission argument that justifies the ping does not exist here. *Publishing the body on a moderation event so a restore can be slotted back in* — it would put a hidden message's text back on the wire, which is the one thing hiding is for; a restore re-opens the conversation instead, one request on a rare event. *Widening the pass to anonymous callers* — see above. *A second Ably connection per store* — spends the ceiling twice as fast for nothing.
- **Consequences:**
  - **`frontend/src/` still contains two 3-second intervals, both deliberate** — `stores/queue.js` (fallback, 2026-09-21) and `stores/chat.js` (fallback **and** the guest path). The sprint's goal sentence "no 3-second poll remains" is met for a signed-in player on a working channel and for nobody else, and this is the entry that says so rather than leaving it implied.
  - **A hidden message disappears for everyone watching without a reload** — for signed-in viewers on the channel immediately, for guests on their next poll.
  - **Chat traffic now counts against the 6M messages/month tier.** It is the highest-volume thing on the channel by some distance, and the first real number will come from the same measurement the sprint already owes for connections.
  - **Nothing about the gates changed.** Posting is still `require_chat_ready`, moderating still admin-only, the rate limit still server-side and checked before anything is published.

---

## 2026-09-21 — One open bet session per room is enforced by a unique index, not by a check the caller can lose
- **Context:** `realtime` Sprint 2 Step 5 and `BACKEND-REVIEW.md` §15. `Queue_Service::sync_turn()` read "is there an open session for this room?" and then inserted one, so any two simultaneous requests could each open a session. The review blamed the 3-second queue polls for the traffic; Sprint 2 Step 2 removed those polls, and the race survived them untouched — `join`, `leave`, `play` and the heartbeat all still call `sync_turn()`, and a busy room is by definition several requests at once. Measured here with 12 truly concurrent processes against the pre-fix logic: **all 12 opened a session for one room.** The cost is not litter. `open_session()` answers the newest open row while the room screen shows the row the queue head points at, so once they diverge a machine payout is banked on a session the player is not looking at — the money is real and the winnings counter never moves.
- **Decision:** `wp_pc_bet_sessions` carries `open_room_id` — the room id while the session is open, `NULL` the moment it closes — under `UNIQUE KEY open_room`. MySQL lets `NULL` repeat in a unique index and nothing else, so the **database** refuses the second open row, and a caller that loses the insert re-reads and adopts the winner's session rather than creating one. `sync_turn()` additionally keeps the head's `session_id` and the room's open session **identical** rather than merely non-empty.
- **Alternatives rejected:** *`SELECT … FOR UPDATE` in a transaction* — locking a row that does not exist yet means relying on InnoDB gap locking, which is invisible in the code, sensitive to isolation level and index choice, and impossible to assert from a single-process `eval-file` check; a money invariant should not rest on something no test can see. *`GET_LOCK()`* — a named lock held across PHP that a killed connection leaks, and nothing in this codebase uses one. *A transient mutex* — not atomic without a persistent object cache, which the FTP shared host does not guarantee. *Deleting the duplicate rows instead of closing them* — they carry coins played, coins won and money won (root `CLAUDE.md` invariant 10). *Leaving the cleanup to a human* — a migration that runs on the version bump is the only thing that reaches an install nobody logs into.
- **Consequences:**
  - **`DATA-MODEL.md` Invariant 9 changes status, not content.** It said "at most one open bet session per room"; it now says what keeps it. This is the same shape as `wp_pc_machine_events.event_key`, the one thing `BACKEND-REVIEW.md` singles out as done right.
  - **Rows written before the bump carry a NULL `open_room_id` and are not covered by the key** until `Install_Schema::migrate_open_sessions()` runs — which is why the migration closes orphans, *then* backfills, *then* verifies the key, in that order. `wp pc queue-sessions` flags any that remain.
  - **A new operator command, `wp pc queue-sessions`.** It reports the rule's own state, not only the rows: an install can look perfectly correct and still be one simultaneous request away from a second session.
  - **A closed session releases the room in the same statement that ends it.** `close_session()` must clear `open_room_id` or the unique key would lock a room against its own next turn — the one way this mechanism can fail closed, and the checks name it.
  - **`pc_db_version` 1.12.0 → 1.13.0.** The first schema change since 1.8.0; the three bumps between were option defaults only.

---

## 2026-09-21 — Operator alerts go out by email, to their own configurable address
- **Context:** `realtime` Sprint 3 Step 1, whose first task is to decide the delivery channel and write it down *before* any alerting code exists. Three alarms are coming this sprint — an unreachable machine during a broadcast, a toss that moved nothing, withdrawals piling up — and the sprint goal is that "an operator finds out from the system rather than from a player writing into support". The step text names the obvious candidate and its flaw in the same breath: email to `pc_support_email` reuses what `Support_Service` already does, but an operator who does not read that inbox gains nothing.
- **Decision:** every operator alert goes out as plain-text email through one function, `Realtime_Alerts::send()`, to **`pc_realtime_alert_email`** — falling back to `pc_support_email`, then to `admin_email`, then to nothing at all. One channel, one call site, one place a second channel would be added.
- **Alternatives rejected:** *Email to `pc_support_email` with no separate option* — one fewer setting, but alerts then land in exactly the inbox the sprint goal wants the operator to stop depending on, and they cannot be moved without moving support mail with them. *A Telegram bot* — very likely the channel the venue operator would actually read, and a genuinely better one; it costs a new external service, a bot token as a wp-config constant, and an account somebody has to create. This project has just spent a sprint discovering what it costs to plan around an account nobody provisioned (`LEARNINGS.md` 2026-09-21; `TECH-STACK.md` still records the Ably peak as unmeasured for that reason), so a channel that works today wins over a better one that waits on a signup. *Recording in the audit log and delivering nothing* — a row nobody reads does not meet the goal, and this sprint's Fixed decisions rule out building the screen that would show it.
- **Consequences:**
  - **No new dependency and no new secret.** `wp_mail` is WordPress core; core rule 1 is not engaged. Nothing is added to `wp-config.php`.
  - **Telegram stays one function away.** `send()` has a single call site per alarm, so a second channel is an edit to `deliver`, not a hunt through the codebase. Revisiting this is cheap on purpose.
  - **Email is not a push notification.** An operator who wants their phone to buzz points `pc_realtime_alert_email` at an address that does that. Said here rather than left to be discovered.
  - **Delivery from the production host is unproven.** The FTP shared host's own mail path has never been confirmed — including for the support-ticket notifications that already depend on it. DDEV's Mailpit proves everything up to the last hop; the last hop is verified by the first real alert after a deploy, or sooner by one test support ticket. This is the main reason the address is configurable rather than fixed.
  - **An alert can never cost anything.** `send()` returns a bool on every path, catches `Throwable`, and an install with no address is a silent no-op — the same shape an unconfigured Ably key already has, and the same rule publishing follows (`FEATURE.md` → Invariants #2).
  - **Two new audit event types**, `operator_alert_sent` and `operator_alert_failed`, carrying the alert's subject and a redacted destination — never the body, which may name a player.

---

## 2026-09-21 — A toss that cannot be judged is counted, never recorded
- **Context:** `realtime` Sprint 3 Step 2 ("a toss that moved nothing"). The step reads: "After a successful toss, the coin sensor is expected to move within a bounded window, as Sprint 1 Step 2 measured. If it does not, record it." What Sprint 1 Step 2 actually measured is narrower, and the spike entry of 2026-09-18 says so twice: the machine resets `sensor.coin` to 0 within 0.09–1.98 s of a toss it accepts, **and** Home Assistant's integration writes only *changed* values, so a toss while the counter already reads 0 re-writes a 0 and leaves no history row at all. The two real presses at 13:16:41Z and 13:17:04Z produced nothing for exactly that reason. In a coin pusher most tosses pay nothing, so the counter sits at 0 for long runs: "the sensor did not move" covers both *the machine did not act* and *nothing here can be known*.
- **Decision:** the record is written **only** where the counter stood above zero at the toss and never moved — the machine failing to reset a counter that had something to reset. Where the counter was already zero (or unreadable) the toss is counted as `unconfirmed` in the pass summary and on `wp pc machine-poll`, and **no row is written**. A `toss_no_movement` row therefore always means one thing: the machine answered 200 and did not act.
- **Alternatives rejected:** *Record every toss with no movement, as the step reads literally* — nothing would ever be missed, but at this venue's rhythm most tosses would carry a record, `toss_no_movement` would become the commonest row in `wp_pc_machine_events`, and an answer to a player built on it would be wrong more often than right. That is the same failure as an alarm that goes off every evening — which this sprint's own goal forbids — moved into a table. *Record both, distinguished by an outcome field in the payload* — everything on file and a dispute answerable either way, at the same row volume, and it asks whoever reads the table to know which outcome means what; the operations dashboard that would make that legible is explicitly a future feature.
- **Consequences:**
  - **The check is blind on a run of fruitless tosses.** After one toss that paid nothing the counter is 0 and stays there, so the next toss cannot be judged either. That blindness is a property of the machine's sensor, not of this code, and no transport can remove it — a repeat is invisible to all of them (`DECISIONS.md` 2026-09-18).
  - **The unconfirmed count is reported, never hidden.** `wp pc machine-poll` prints it next to the judged and recorded counts, so "nothing recorded" is never silently confused with "all clear".
  - **The step's own test wording is met in spirit, not to the letter:** "a toss followed by no move raises exactly one record" holds for every toss where *no move* can be observed at all. The plan put this to the user as its only open question and it was approved as recommended.
  - **It is a record, not an alert.** The step asks for a record and names no notification; `Realtime_Alerts` (S3.1) is one call site away when a step asks for one.
  - **A future sensor could reopen this.** If the machine ever gains an entity that changes on every toss — a press counter, a motor state — the unobservable case disappears and the rule can be widened. Nothing about the record's shape would change.

---

## 2026-09-22 — A backlog alert needs a crossing *and* the period, and never reminds
- **Context:** `realtime` Sprint 3 Step 3 ("withdrawals piling up"). The step asks for two things in one breath: "alert at most once per period, so a busy day does not become a stream of notifications" and "re-alert only after dropping below and crossing again". Read as alternatives they disagree — a period that re-alerts while a backlog persists is a reminder, which the second sentence forbids; a latch alone lets a queue hovering on the threshold clear and re-cross all afternoon, mailing each time, which the first forbids. Unlike the machine alarms beside it, this one watches a human process: nothing breaks when withdrawals pile up, so there is no failure anywhere to rate-limit against.
- **Decision:** both gates must open for an alert to go out — the **latch** (one alert per pile-up, cleared only when the count *and* the age fall back under their thresholds, which is audited `withdrawal_backlog_cleared` and is what arms the next alert) and the **period ceiling** (`pc_realtime_withdrawal_alert_period_seconds` since the last alert actually *sent*, remembered across a clearing). Nothing reminds and nothing escalates. A crossing the ceiling suppresses is **delayed, not lost**: the watch keeps looking and alerts on the first pass after the period elapses. And the latch records the decision to alert, never the delivery — a refused `wp_mail` still latches, and `Realtime_Alerts` audits the refusal.
- **Alternatives rejected:** *The period as a reminder* — re-alerting every period while the backlog lasts would suit an operator on holiday, and is exactly what "re-alert **only** after dropping below and crossing again" rules out; escalation is a business decision nobody has made (`ROADMAP.md` Phase 8). *The latch alone, no ceiling* — one flapping queue at the threshold becomes a stream of mail, which is the failure the step names first. *Latching on a suppressed crossing* — it would make the ceiling swallow a genuine backlog for good rather than delay it; the outage watch already had this shape decided the other way (2026-09-21, an outage that starts outside a broadcast window alerts when the window opens).
- **Consequences:**
  - **Where the two rules disagree the watch stays quiet.** Same direction as every judgement in Step 2: an alarm nobody can trust is worse than one that speaks a little late.
  - **A backlog that is never cleared is alerted once, ever** — not once a day. The clearing is the arming, and `withdrawal_backlog_cleared` in the audit log is how an operator sees that it happened.
  - **The state option is what makes this work**, and `last_alert_at` deliberately survives a clearing. Deleting `pc_realtime_withdrawal_alert_state` re-arms everything, which is how a check or an operator forces an alert.
  - **Both thresholds at `0` switch the watch off** — the empty configuration is the off switch, as with the parked captcha and Google sign-in. Nothing is commented out.

---

## 2026-09-22 — The withdrawal watch rides the machine poll pass rather than a cron of its own
- **Context:** Same step. The backlog has nothing to do with the machine — it reads one indexed aggregate over `wp_pc_transactions` and never calls Home Assistant — so the obvious home for it is its own scheduled event rather than a pass named `machine-poll`.
- **Decision:** run it as the fourth watch inside `Machine_Poller::run()`, after the toss watch, inside the `try` and before the machine-id and ingest-secret guards.
- **Alternatives rejected:** *Its own WP-Cron event* — WP-Cron fires on web traffic, and the hosts that need this watch most are the quiet ones; worse, a host with a real cron drives `wp pc machine-poll` directly, so a second schedule would tick on exactly the installs that have no real cron. *A cron line of its own in the host's crontab* — a second thing for an operator to install correctly, for a query that costs less than the pass it would run beside. *Checking on a request, as queue pruning does* — the backlog is not healed by traffic and nobody visits the admin SPA on the days this matters.
- **Consequences:**
  - **`wp pc machine-poll` now reports something that is not about the machine.** The command's job is "what the pass did", and the pass does this too; the `withdrawals:` line prints on every pass, including the quiet ones, because "nobody is waiting" is the answer an operator wants and silence cannot be told from "never checked".
  - **The poll interval is now also the backlog's resolution.** At the default 60 s that is far finer than a threshold measured in days, and `pc_realtime_poll_interval_seconds` is the one knob for both.
  - **Four watches now share one pass**, all outside the transport's guards, and each one's failure is contained to itself. A fifth would go in the same place for the same reasons.
