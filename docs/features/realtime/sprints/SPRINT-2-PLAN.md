...
### Execution notes
- **Tasks 1–4 ran as written**, one commit each in `backend/` or `frontend/` plus its docs commit.
- **One file was touched that the plan's file list did not name:** `frontend/src/services/chatService.js` now *exports* its `mapMessage`, so a pushed message and a fetched one produce exactly the same object. No behaviour changed — the alternative was a second copy of the message shape inside the store, which is two places to get wrong. Disclosed here rather than folded in silently.
- **Checks that fail on the pre-step code:** six of the moderation checks (nothing published a `moderation` message before this step). The chat-publish checks likewise. Verified by reverting each controller in turn and re-running.
- **One check was wrong when first written** and was fixed in place: the rate-limit check compared the number of publishes against a cursor read whose cursor was wrong, so it counted the room's whole history. It now counts the accepted posts in the flood loop directly, which is what it was trying to say.
- **Cleanup measured, not assumed:** the local database was counted before and after a run — audit rows, messages, wallets, coin lots and users were all identical. The pre-existing orphans from earlier steps' scripts are untouched and still want the `/adhoc` already recorded.
- **Gate:** `backend/bin/check` exit 0 before every backend commit (66 php files; `realtime-chat.php` in the executed list) and `frontend/bin/check` exit 0 before every frontend commit — `lint: OK`, `build: OK`.
