...
- [x] Sprint boundary: the sprint's work merged into `main` per the git model, `backend` `main` pushed (the production FTP release) and `frontend` `main` pushed (the Vercel build). `PC_STRIPE_SECRET_KEY` and `PC_STRIPE_WEBHOOK_SECRET` exist in production `wp-config.php` and the production webhook URL is registered in the Stripe Dashboard **before** the merge — otherwise the first release answers `stripe_not_configured`. — merge: docs `6da0126`, backend `75450469`, frontend `7210c59`, admin `c40773a`; the pushes and the server-side configuration **confirmed by user 2026-09-18** (not repository-evidenced — see `SPRINT-1-CLOSE.md` → Definition of Done)
- [x] Tymofii paid with a test card on the deployed stack and saw the coins in the wallet without reloading anything by hand — confirmed by user 2026-09-18
- [x] The same event resent from the Stripe Dashboard credited nothing a second time, observed in that player's balance — confirmed by user 2026-09-18; the local equivalent was observed in Step 4 (`stripe events resend`, balance unchanged, `stripe_webhook_already_settled`)
