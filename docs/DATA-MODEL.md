...
| `ROOM_MACHINE_ID` | `pc_room_machine_id` | string | Maps the room to a Home Assistant machine. The link that makes a machine event attributable. At most one non-trashed `available` room may carry a given non-empty id. `POST`/`PUT /admin/rooms` refuse a second with `machine_already_in_use`, and `wp pc machine-rooms` reports what already exists (`Machine_Rooms`, `realtime` Sprint 1 Step 3). An empty id claims nothing. |
